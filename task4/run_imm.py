...
init_state_CV = GaussParams(
    init_mean[:4], init_cov[:4, :4])  # get rid of turn rate
...
measurement_model_CT = measurementmodels.CartesianPosition(
    sigma_z, state_dim=5)
...
# copy of the two modes
init_mode_states = [GaussParams(init_mean, init_cov)] * 2
...
ratio_in_CI = np.sum(np.less_equal(
    CINIS[0], NIS) * np.less_equal(NIS, CINIS[1])) / K
CI_LABELS = ["CI0", "CI1"]
...
x_bar_modes = np.array([[comp.mean for comp in pr.components]
                        for pr in imm_preds])
P_bar_modes = np.array([[comp.cov for comp in pr.components]
                        for pr in imm_preds])

x_hat_modes = np.array([[comp.mean for comp in pr.components]
                        for pr in imm_upds])
P_hat_modes = np.array([[comp.cov for comp in pr.components]
                        for pr in imm_upds])
...
devstr = ", ".join(f"{num:.3f}" for num in (
    pos_peak_deviation, vel_peak_deviation))
# plot
...
ratio_in_CI = np.mean(np.less_equal(
    CINIS[0], NIS) * np.less_equal(NIS, CINIS[1]))
